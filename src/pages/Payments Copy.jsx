...
const Payments = () => {
  ...
  const { token, user, logout, authFetch } = useAuth(); // <-- Added authFetch
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      // <-- Handle thrown "Unauthorized" - no alert, already redirected
  ...
  const fetchBranches = async () => {
    ...
    try {
      const response = await authFetch(`${API_BASE_URL}/branches`); // <-- Switched
  ...
  const fetchPaymentById = async (paymentId) => {
    try {
      const response = await authFetch(`${API_BASE_URL}/payments/${paymentId}`); // <-- Switched
  ...
  // Updated: handleDeletePayment uses authFetch + try-catch
